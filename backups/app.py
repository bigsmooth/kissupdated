...
def login(username, password):
    # Fetch stored hash
    rows = query("SELECT username, password, role, hub FROM users WHERE username=?", (username,))
    if not rows:
        return None
    uname, stored_hash, role, hub = rows[0]
    ...
    try:
        if stored_hash and stored_hash.startswith("$2"):
            ...
            if bcrypt.checkpw(password.encode(), stored_hash.encode()):
                # Optional: upgrade legacy formats here (already bcrypt, so skip)
                return (uname, role, hub)
            return None
    except Exception:
        # If bcrypt import fails and hash is bcrypt -> cannot verify
        return None

    # Legacy SHA-256 path (64 hex chars)
    import re, hashlib
...
def unread_count(username: str) -> int:
    """Return count of unread messages for a given user."""
    try:
        rows = query("SELECT COUNT(*) FROM messages WHERE recipient=? AND read_at IS NULL", (username,))
